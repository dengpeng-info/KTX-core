...
describe("Timelock", function () {
  ...
  it("mint", async () => {
    ...
    await expect(
      timelock.connect(user0).mint(ktx.address, 900)
    ).to.be.revertedWith("Timelock: forbidden");

    await expect(
      timelock.connect(wallet).mint(ktx.address, 900)
    ).to.be.revertedWith("BaseToken: forbidden");

    await ktx.setGov(timelock.address);

    expect(await ktx.isMinter(timelock.address)).eq(false);
    expect(await ktx.balanceOf(mintReceiver.address)).eq(0);

    await timelock.connect(wallet).mint(ktx.address, 900);

    expect(await ktx.isMinter(timelock.address)).eq(true);
    expect(await ktx.balanceOf(mintReceiver.address)).eq(900);

    await expect(
      timelock.connect(wallet).mint(ktx.address, expandDecimals(1001, 18))
  ...
  it("setMaxStrictPriceDeviation", async () => {
    ...
    await timelock
      .connect(user0)
      .setMaxStrictPriceDeviation(vaultPriceFeed.address, 200);
